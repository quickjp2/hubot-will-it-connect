...
process.env.ABC_FOUNDATION = "http://willitconnect.abc.io"
...
describe 'will-it-connect', ->
  ...
  context 'user makes a bad request', ->
    ...
    it 'and it should reply with an error message for a request without a port',  ->
      ...
      expect(room.robot.emit.lastCall.args[1].content.title).equals("willItConnect: :null")
  ...
  context 'user request for a valid url and port combination using specific willitconnect instance', ->
    beforeEach (done) ->
      room.robot.emit = sinon.spy()
      room.user.say 'alice', "hubot in ABC willitconnect " + goodUrl + ":" + goodPort
      setTimeout done, 100
